 ...
 *  Description:  用于DHCP模式下，调用本函数获取本机IP并写入应答报文中。
 * =====================================================================================
 */
int set_device_new_ip()
{
    ...
    local_gateway = ((struct  sockaddr_in*)&ifr.ifr_addr)->sin_addr.s_addr;
    ...
    local_dns = ((struct sockaddr_in*)&ifr.ifr_netmask)->sin_addr.s_addr;
    return 0;
}
...
{
    char buf[64];
    printf("######## ZDClient ver. %s $Revision: 1.2.2 $ #########\n", ZDC_VER);
    printf("Device:     %s\n", dev_if_name);
    if (dhcp_on == 0 )
	{printf("IP Mode:    %s\n", "user mode");}
    else
